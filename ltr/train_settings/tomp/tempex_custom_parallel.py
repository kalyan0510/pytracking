...
def run(settings):
    ...
    # scale w.r.t the bbox. 4.5f means the center can drift img_sz*4.5f
    settings.center_jitter_factor = {'train': 0., 'test': 4.5}
    # https://www.desmos.com/calculator/yc98cuxzaw
    # the scaling uses e^(scale_fac*rand(mean=0, var=1)) effectively zooming
    # in range .6 to 1.64 factors in rand range(-1 to 1)
    ...
    settings.test_sequence_length = 30 # CHANGE THIS BEFORE TRAINING
    settings.num_encoder_layers = 6
    settings.num_decoder_layers = 6
    settings.frozen_backbone_layers = ['conv1', 'bn1', 'layer1', 'layer2'] # LOOK AT THIS BEFORE TRAINING
    settings.freeze_backbone_bn_layers = True # LOOK AT THIS BEFORE TRAINING
    ...
    def modify_tompnet_state_dict(ordered_state_dict):
        def update_key(key):
            if 'feature_extractor' in key and 'head' not in key:
                return key.replace('feature_extractor', 'feature_extractor.module')
            return key
        return OrderedDict([(update_key(k), v) for k, v in ordered_state_dict.items()])

    # settings.modify_net_state_dict = None

    # # Train datasets
    # print('loading LaSOT')
    # lasot_train = Lasot(settings.env.lasot_dir, split='train')
    # # print('loading Got10k')
    # # got10k_train = Got10k(settings.env.got10k_dir, split='vottrain')
    # print('loading TrackingNet')
    # trackingnet_train = TrackingNet(settings.env.trackingnet_dir, set_ids=list(range(4)))
    # # coco_train = MSCOCOSeq(settings.env.coco_dir)
    #
    # # Validation datasets
    # got10k_val = Lasot(settings.env.lasot_dir, split='train')#Got10k(settings.env.got10k_dir, split='votval')
    #
    # Train datasets
    print('loading LaSOT')
    lasot_train = Lasot(settings.env.lasot_dir, split='train')
    ...
    # Train sampler and loader
    dataset_train = sampler.KYSSampler([trackingnet_train, got10k_train, lasot_train],
                                       [1, 1, 1],
    ...
    actor = actors.ParallelMyToMPActorTempExtended(net=net, objective=objective, loss_weight=loss_weight, multi_gpu=settings.multi_gpu)
    print("Actor created")
    ...
    print("Trainer created and is now initiated")
